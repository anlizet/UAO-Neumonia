...
import tkcap
import time
import numpy as np  # Importamos numpy para generar datos simulados

# -----------------------------
#   Función de Predicción Simulada
def predict(input_array):
    # Esta función simula una predicción básica
    label = "Neumonía Detectada"
    proba = np.random.uniform(80, 99)  # Simulación de probabilidad alta
    heatmap = np.random.rand(512, 512)  # Simulación de heatmap aleatorio
...
class App():
    def __init__(self):
        ...
        self.style.configure('TButton', font=('Helvetica', 10, 'bold'), foreground='black', background='white')
    ...
    def run_model(self):
        if self.array is not None:
            # Llamada a la función predict
            self.label, self.proba, self.heatmap = predict(self.array)
            # Código para mostrar la imagen y los resultados en la interfaz gráfica
            self.img2 = Image.fromarray((self.heatmap * 255).astype(np.uint8))  # Convertir heatmap a imagen
    ...
    def save_results_csv(self):
        with open('historial.csv', 'a', newline='') as csvfile:
            w = csv.writer(csvfile, delimiter=',')  # Usamos ',' como delimitador estándar para CSV
